...
CatalogController.category = async (ctx, next) => {
    ...
    const list = await Category.findAll({where: {parent_number: ctx.params.sparePartId}});

    console.log(ctx.params.sparePartId);

    ctx.body = await ctx.render('catalog/category', {model, 'spare_part': sparePart, list})
}
...
CatalogController.product = async (ctx, next) => {
    ...
    const images = await Image.findAll(
        {
            attributes: ['id', 'hash'],
        where: {parent_number: sparePart.parent_number, number: sparePart.number, model_id: model.id},
            include: { model: Product, as: 'products' }
        });

    console.log(JSON.stringify(images));

    ctx.body = await ctx.render('catalog/product', {model, 'spare_part': sparePart, images})
}